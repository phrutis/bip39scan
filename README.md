## bip39scan - Brute Pool
![o1AJ9qDyyJNSpZLSMmjZp1oHDUkVhqVqHnDVdzwQYjHMFBpSr](https://github.com/phrutis/bip39scan/assets/140947743/5e506934-c615-4fa9-aa1a-d3a41e16ff5f)

### The Bip39Scan Pool is Coming Soon!

### Stay tuned to this repo for updates!

This pool will be the fastest search pool in the world, using the latest GPU libraries and having a programmer stand by the project through testing.<br>

We managed to achieve a speed of 3090 = 5M/s (5000000 seed/s)<br>

We will be looking for accounts from $100,000 and above.<br>

If you are ready to search with us 24/7, join us.<br>

Telegram: https://t.me/+k1jKsx_fxTo1MzQx<br>
Launch, Connection and Questions will be posted in Group.<br>

Miners to share 70% of find based on contribution.<br>
There are no finder rewards since this works on a group effort.

Organizers 30%

% of the pool may be made after a major find..<br>

## bip39scan - брут мнемонических фраз bip39.

### Скоро запуск пула ⏳

### Следите за обновлением этого репозитория

🚀 Это самым быстрый пул в мире. 🚀<br>
В программе используются быстрые алгоритмы и библиотеки.<br>
Нам удалось достичь скорости RTX 3090 = 5М/с (5000000 seed/сек)<br>
Будем искать счета от $100000 и выше в режиме рандом<br>
У рандома больше шансов найти.

### Почему наш пул лучше других 🥇<br>
1️⃣ Высокая скорость поиска RTX 3090 = 5M/s.<br>
2️⃣ Нам удалось снизить расход электричества на 85-90%.<br>
Карта в пуле потребляет всего 10-15% от номинала и не греется!*<br>
<sub>*(потребление тестировалось на RTX 4090 и 3090 на других картах нужны доп. тесты)</sub>

Майнеры могут попробовать запустить майнинг + пул.<br>
Возможно придётся понизить хешрейт майнинга.<br>
Нужны тесты и регулеровка, всё зависит от вашего оборудования.

### Условия выплат 💰
70% баланса находки выплачиваются тому кто нашел мнемоническую фразу.<br>
30% баланса получают организаторы пула.<br>

Если вы готовы искать вместе с нами 24/7, присоединяйтесь.<br>
Telegram: https://t.me/+k1jKsx_fxTo1MzQx<br><br><br>

### Часто задаваемые вопросы ❓ 

Сделайте % от пула всем участникам.<br>
Давайте так и так или так.

Ответ:<br>
В перспективе рассматривается вариант 50/30/20.<br>
50% - тому кто нашел.<br>
30% - организаторам.<br>
20% - делится между всеми учасниками исходя из количества пройденных фраз (Total)<hr>






